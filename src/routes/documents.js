// src/routes/documents.js (Enhanced Version)
...
const upload = multer({
  ...
  fileFilter: (req, file, cb) => {
    ...
    if (file.mimetype === 'application/pdf') {
      // Additional size check in filter
...
router.get('/', authMiddleware, async (req, res) => {
  try {
      ...
      documents.map(async (doc) => {
        ...
        const docWithProgress = {
          ...
          intelligent_insights: {
            ...
            suggested_session_length: this.calculateSuggestedSessionLength(doc, pages),
            difficulty_trend: this.analyzeDifficultyTrend(pages),
            completion_prediction: this.predictCompletionDate(doc, pages, completedPages)
          }
        };
        ...
        if (include_analysis === 'true') {
          ...
          docWithProgress.detailed_analysis = {
            ...
            content_distribution: this.analyzeContentDistribution(analysisData),
            difficulty_progression: this.analyzeDifficultyProgression(analysisData)
          };
        }
    ...
    const collectionStats = {
      ...
      difficulty_distribution: this.calculateDifficultyDistribution(documentsWithProgress),
      content_type_distribution: this.calculateContentTypeDistribution(documentsWithProgress),
      completion_statistics: this.calculateCompletionStatistics(documentsWithProgress),
...
// Enhanced document details with comprehensive analysis
router.get('/:id', authMiddleware, async (req, res) => {
  try {
    ...
    const progressMetrics = this.calculateComprehensiveProgress(document, pages, contentAnalysis);
    ...
    if (include_recommendations === 'true') {
      ...
      recommendations = await this.generateDocumentRecommendations(
        document, 
        pages, 
        contentAnalysis, 
        userReadingSpeed,
    ...
    const response = {
      document: {
        ...
        content_analysis_summary: this.summarizeContentAnalysis(contentAnalysis),
        reading_path: this.generateOptimalReadingPath(pages, contentAnalysis)
      },
      pages: pages || [],
      study_sessions: studySessions,
      recommendations: recommendations,
      insights: {
        learning_velocity: this.calculateLearningVelocity(pages, studySessions),
        focus_patterns: this.analyzeFocusPatterns(studySessions),
        optimal_study_times: this.suggestOptimalStudyTimes(studySessions),
        difficulty_adaptation: this.assessDifficultyAdaptation(pages, contentAnalysis)
      }
    };
...
// Document analysis endpoint for deep insights
router.get('/:id/analysis', authMiddleware, async (req, res) => {
  try {
    ...
    const { data: document } = await supabase
      .from('documents')
      .select('id, title, total_pages, processing_metadata')
      .eq('id', req.params.id)
      .eq('user_id', req.user.id)
      .single();
    ...
    const analysis = {
      document_overview: {
        title: document.title,
        total_pages: document.total_pages,
        analysis_version: document.processing_metadata?.api_version || '1.0.0',
        last_analyzed: document.processing_metadata?.processedAt
      }
    };

    switch (analysis_type) {
      case 'difficulty':
        analysis.difficulty_analysis = this.generateDifficultyAnalysis(contentAnalysis, pages);
        break;
        
      case 'content':
        analysis.content_analysis = this.generateContentAnalysis(contentAnalysis);
        break;
        
      case 'progress':
        analysis.progress_analysis = this.generateProgressAnalysis(pages, contentAnalysis);
        break;
      ...
      default:
        ...
        analysis.content_analysis = this.generateContentAnalysis(contentAnalysis);
        analysis.progress_analysis = this.generateProgressAnalysis(pages, contentAnalysis);
        analysis.recommendations = await this.generateAnalysisRecommendations(
          document.id, 
          req.user.id, 
          contentAnalysis, 
          pages
...
// Re-analyze document endpoint (for updating old documents)
router.post('/:id/reanalyze', authMiddleware, async (req, res) => {
  try {
    ...
    if (downloadError) {
      return res.status(500).json({ 
        error: 'Failed to download document for reanalysis',
    ...
    // Save new analysis
    await pdfProcessor.savePDFAnalysisToDatabase(document.id, req.user.id, analysisData);
    ...
    res.json({
      message: 'Document reanalyzed successfully with enhanced intelligence! 🧠',
      analysis_summary: {
        total_words: analysisData.documentMetrics.totalWords,
        average_difficulty: analysisData.documentMetrics.averageDifficulty,
        structural_complexity: analysisData.documentMetrics.structuralComplexity,
        estimated_reading_hours: Math.round(analysisData.timeEstimates.totalSeconds / 3600 * 10) / 10
      },
      improvements_detected: this.compareAnalysisVersions(document.processing_metadata, analysisData.processingMetadata),
      new_recommendations: recommendations.slice(0, 3)
    });

  } catch (error) {
    console.error('Document reanalysis error:', error);
    res.status(500).json({ error: 'Failed to reanalyze document' });
  }
});
