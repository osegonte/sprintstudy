...
router.patch('/:id/activity', authMiddleware, async (req, res) => {
  try {
    ...
    const { data: session } = await supabase
      .from('study_sessions')
      .select('started_at, total_duration_seconds, active_reading_seconds')
      .eq('id', id)
      .eq('user_id', userId)
      .single();
    ...
    const activeTime = currentDuration - break_time_seconds;
    ...
    const { data: updatedSession, error } = await supabase
      ...
      .update({
        ...
        active_reading_seconds: Math.max(0, activeTime),
...
router.patch('/:id/pause', authMiddleware, async (req, res) => {
  try {
    const { id } = req.params;
    const { reason = 'break' } = req.body; // break, interruption, other
    ...
    const pauseData = session.pause_data ? JSON.parse(session.pause_data) : { pauses: [] };
...
function generateRealTimeSessionFeedback(session, focusScore) {
  const durationMinutes = Math.round(session.total_duration_seconds / 60);
  const pagesPerMinute = session.pages_covered / (durationMinutes || 1);
  ...
  } else if (pagesPerMinute < 0.5 && session.pages_covered > 0) {
...
function generateActivityRecommendations(session) {
  ...
  const durationMinutes = session.total_duration_seconds / 60;
  const focusScore = session.focus_score || 0.7;

  // Duration-based recommendations
  if (durationMinutes >= 45 && !session.break_time_seconds) {
...
function calculateRemainingOptimalTime(session) {
  const currentDuration = session.total_duration_seconds / 60;
...
async function updateDailyAnalyticsFromSession(userId, date, session) {
  // This would update the reading_analytics table with daily aggregates
  // Implementation similar to the analytics route updateDailyAnalytics function
  console.log(`📊 Would update daily analytics for ${userId} on ${date}`);
}
...
function analyzeSessionPerformance(session, detailed = false) {
  ...
  const pagesPerMinute = pages > 0 ? duration / pages / 60 : 0;
...
function calculateSessionStatistics(sessions) {
  ...
  const avgFocus = completedSessions.reduce((sum, s) => sum + (s.focus_score || 0.7), 0) / (completedSessions.length || 1);
...
function generateSessionInsights(session, feedback) {
  const insights = [];
  const focusScore = session.focus_score || 0.7;
  const duration = session.total_duration_seconds / 60;
  ...
  if (feedback && feedback.length > 0) {
    const encouragingFeedback = feedback.filter(f => f.encouragement_level >= 4).length;
